...
pub fn print_stats_dashboard(stats: &[(String, u64)]) {
    ...
    writeln!(
        out,
        "\x1b[1;36m║        AegisShield XDP Live Dashboard         ║\x1b[0m"
    )
    .ok();
    lines += 1;
    writeln!(
    ...
    for (name, value) in stats {
        let color = if *value > 0 && name.contains("drop") {
            "\x1b[1;31m" // Red for drops
        } else if *value > 0 && name.contains("pass") {
            "\x1b[1;32m" // Green for passes
        } else {
            "\x1b[1;33m" // Yellow for neutral/0
        };
        writeln!(
            out,
            "\x1b[1;36m║\x1b[0m  {:<25} {}{:>15}\x1b[0m \x1b[1;36m║\x1b[0m",
            name,
            color,
            format_number(*value)
        )
    ...
    writeln!(
        out,
        "\x1b[1;36m║  \x1b[0mPress Ctrl-C to stop                         \x1b[1;36m║\x1b[0m"
    )
    .ok();
    lines += 1;
    writeln!(
